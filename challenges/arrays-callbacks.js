...
// Given this zoo data from around the United States, follow the instructions below.  Use the specific array methods in the
//requests below to solve the problems.
...
zoos using the .reduce() method. Remember the reduce method takes two arguments: a callback (which itself takes two args), 
and an initial value for the count.

*/
let populationTotal = zooAnimals.reduce((totalPop, current) => {
  return (totalPop += current.population);
}, 0);
...
console.log(consume(1, 4, multiply));
console.log(consume("Dan", "Smith", greeting));
function add(a, b) {
  return a + b;
}
